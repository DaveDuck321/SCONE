...
def read_generation_positions_from_file(file):
    ...
    floating_point_rx = re.compile('[-+]? (?: (?: \d* \. \d+ ) | (?: \d+ \.? ) )(?: [Ee] [+-]? \d+ ) ?', re.VERBOSE)
...
scatter = plt.scatter(zeros, zeros, s=0.1)

def animate_plot(frame):
    scatter.set_offsets(positions[frame, :, :])

anim = animation.FuncAnimation(fig, animate_plot, frames=range(len(positions)))
